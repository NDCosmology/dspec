...
int force_treebuild_single(int npart)
{
  ...
  int nfree;                                  //Index for free nodes
	int th;																			//Index for Nodes[]
	int nn;                                     // nn = Nodes[th].u.suns[*]
	int no;                                     //Index for TopNode[] & DomainNodeIndex[]
  struct NODE *nfreep;												//Free nodes
  double lenhalf;
  double epsilon;                             //Force Softening
  peanokey key;
  ...
  /* now we insert all particles. ALI: For each individual processor */
  for(i = 0; i < npart; i++)
  {
    ...
    //Calculate w/r/t lower corner of simulation volume    
    ...
    //Walk TopNode[], find which TopNode P[i] should be located in  
    no = 0;
    while(TopNodes[no].Daughter >= 0)
	    no = TopNodes[no].Daughter + (key - TopNodes[no].StartKey) / (TopNodes[no].Size / 8);

    no = TopNodes[no].Leaf;
    th = DomainNodeIndex[no];              //th is always > DomainNodeIndex[no]  

    while(1)
	  {
	    if(th >= All.MaxPart)	/* we are dealing with an internal node */
	    {
			  //Determine if P[i] is a subnode or not. 
	      subnode = 0;
	      ...
	      nn = Nodes[th].u.suns[subnode];           // = index of P[i]  

	      if(nn >= 0)	/* ok, something is in the daughter slot already, need to continue */
		    {
		      parent = th;
		      th = nn;							//Restart search at index 'th'  
		    }
	      else
		    {
	    ...
	    else
	    {
	       ...
	       */
				//Point Parent Node to Free Node (w/in the internal nodes).         
        ...
        //Point newly created internal node to the particle,'P[i]' to  index, 'i' 
	      subnode = 0;
          ...
          {
		        /* seems like we're dealing with particles at identical 
               (or extremely close) locations. Randomize subnode index to allow 
               tree construction. Note: Multipole moments of tree are still correct, 
...
*/
void force_create_empty_nodes(int no, int topnode, int bits, int x, int y, int z, 
                              int *nodecount, int *nextfree)
{
  int i, j, k, n;
  int sub;	 														//Goes from 0 -> 7. Sub 'topnode'
  int count;														//Index for *.u.suns().  Goes from 0 -> 7
    ...
    for(i = 0; i < 2; i++)
      for(j = 0; j < 2; j++)
	      for(k = 0; k < 2; k++)
	      {
	        ...
	        Nodes[no].u.suns[count] = *nextfree;			//Store next free nodes' index
	        ...
	        *nodecount = *nodecount + 1;
					...
					//If end of Nodes is over-stepped... End Run
...
void force_update_node_recursive(int no, int sib, int father)
{
  ...
  {
    for(j = 0; j < 8; j++)
	    suns[j] = Nodes[no].u.suns[j];	/*this "backup" is necessary because the nextnode
 ...
 *  table.
 *
 *  ALI: I believe that the force contributions from the mesh are included
 *  in this function.
 */
int force_treeevaluate_shortrange(int target, int mode)
{
  struct NODE *nop = 0;						//use to access tree nodes
  int no;													//Index for Nodes[]
	int ptype;											//Target Particle type
 	int ninteractions, tabindex;    //Number of particles target interacts w/
  double dx, dy, dz;              //Dist. between target and other nodes
  double r2;                      // = dx^2 + dy^2 + dz^2
	double r;												// = (r2)^0.5
  double mass;										//mass of force node (i.e. not the target)
  double fac;                     // (r>=h) ? mass/r^3 : 
  double u;                       // = (r/h)
  double h;												//Force Softening
  double h_inv;										// = 1/h
	double h3_inv;									// = (1/h)^3
  double acc_x, acc_y, acc_z;     //Acceleration of target particle
  double pos_x, pos_y, pos_z;     //Position of target particle
  double aold;										//Mag. of old grav. force, Used in opening criterion
  double eff_dist;                //Use to check if node is outside rcut
  double rcut;                    //Max radius of short-range tree force 
  double asmth;                   //=All.Asmth[0] = ASMTH * All.Boxsize / PMGRID
  double asmthfac;                //=0.5/asmth*(NTAB/3.0) (ie. (1/(2*r_s) see eqn. 23)
  double rcut2;                   //rcut^2
  double dist;   
  #if defined(UNEQUALSOFTENINGS) && !defined(ADAPTIVE_GRAVSOFT_FORGAS)
    int maxsofttype;                  //
  #endif
  #ifdef ADAPTIVE_GRAVSOFT_FORGAS
    double soft = 0; 										//Sph smoothing length
  #endif
  #ifdef PERIODIC
    double boxsize, boxhalf;
    boxsize = All.BoxSize;
  ...
  if(mode == 0)
  {
		//Get position of target particle
    pos_x = P[target].Pos[0];		
    pos_y = P[target].Pos[1];
    pos_z = P[target].Pos[2];
    ptype = P[target].Type;
  ...
  #endif
  rcut2 = rcut * rcut;
  asmthfac = 0.5 / asmth * (NTAB / 3.0);      //Related to 1/(2*r_s) in eqn. 21 

  //Force softening of target particle
  #ifndef UNEQUALSOFTENINGS
    h = All.ForceSoftening[ptype];
    h_inv = 1.0 / h;
    h3_inv = h_inv * h_inv * h_inv;
  ...
  /* Start at root node, walk the tree. Recall that the internal nodes are 
     All.MaxPart <= no < All.MaxPart + MaxNodes.  Starting with the root Node[], 
     it checks to see if the Node[*] is within the cut off radius (r^2 < rcut^2).
     If true it continues walking the Nodes[] daughters. If false, it checks
		 to ensure that the node is completely outside of the cut off radius (recall
     r^2 is w/r/t the center of the node). 

     If the node is outside rcut AND greater than 'rcut + 0.5 * node->length', 
     no is advanced to the next sibling (i.e. no = nop.u.d.sibling). We also 
     check the 3 different openning criterion to determine if we should continue
     openning more nodes. Also checks to see that r < softening length, if true
     and if multiple particles have different softening lengths, it opens  
     the next node.

		 These nodes are walked until individual particles are reached, then the 
		 accelerations are calculated. The many 'continue' statements in the 
     no > All.MaxPart section ends that iteration of the loop. The loop then 
     restarts w/ the new node indice 'no'. 
		 
   */
  while(no >= 0)
  {
	  /* the index of the NODE is the index of the particle,NOT internal NODE */
    if(no < All.MaxPart)
    {
	    dx = P[no].Pos[0] - pos_x;
	    dy = P[no].Pos[1] - pos_y;
    ...
    else			/* we have an internal node, All.MaxPart <= no */
    {
	    if(no >= All.MaxPart + MaxNodes)	/* pseudo particle */
	    {
	      if(mode == 0)
      ...
      //Get distance from target particle to center of node
      dx = nop->u.d.s[0] - pos_x;
      dy = nop->u.d.s[1] - pos_y;
      dz = nop->u.d.s[2] - pos_z;
      #ifdef PERIODIC
      ...
      r2 = dx * dx + dy * dy + dz * dz;
			...
			//Is node completely outside cut-off radius? Yes: advance 'no' to next sibling
      if(r2 > rcut2)
      {
	      /* check whether we can stop walking along this branch */
	      eff_dist = rcut + 0.5 * nop->len;
        #ifdef PERIODIC
	        dist = NEAREST(nop->center[0] - pos_x);        //X coordinate
        #else
        ...
        #ifdef PERIODIC
	        dist = NEAREST(nop->center[1] - pos_y);        //Y coordinate
        #else
        ...
        #ifdef PERIODIC
	        dist = NEAREST(nop->center[2] - pos_z);        //Z coordinate
        #else
	      ...
	      if(dist < -eff_dist || dist > eff_dist)
        {
			...
			//Check opening criterion: BH, eqn 18, and eqn 19 in GADGET2 paper
	    if(All.ErrTolTheta)	/* check Barnes-Hut opening criterion */
	    {
				//Using (l/D)^2 < theta^2 in notation of BH
	      if(nop->len * nop->len > r2 * All.ErrTolTheta * All.ErrTolTheta)
		    {
		      /* open cell */
		      no = nop->u.d.nextnode;
	    ...
	    {
	      if(mass * nop->len * nop->len > r2 * r2 * aold)   //eqn. 18
	 	    {
		      /* open cell */
		      no = nop->u.d.nextnode;
		      continue;
	      ...
	      /* check in addition whether we lie inside the cell. Eqn. 19*/
	      if(fabs(nop->center[0] - pos_x) < 0.60 * nop->len)
		    {
		      if(fabs(nop->center[1] - pos_y) < 0.60 * nop->len)
		      {
          ...
          {
            //Set 'h' to max soft. length, presumably of the node's particles 
            if(h < All.ForceSoftening[maxsofttype])
            {
              h = All.ForceSoftening[maxsofttype];
              if(r2 < h * h)                      //Need higher resolution
              {
                if(((nop->u.d.bitflags >> 5) & 1))	/* bit-5 signals that there are particles of different softening in the node */
                {
                  no = nop->u.d.nextnode;             
    ...
    r = sqrt(r2);
		...
		/* Compute accelerations. Convert into internal units later (see gravitytree.c)
			 This is somehow the smoothing function W(r,h) from eqn. 4.  I am pretty darn
       sure of it. I just can't get the constants to work out. See eqn below Eqn 28.
			 Recall, (dx, dy, dz)/r gives the vector direction.
    */
    if(r >= h)
    {
	    fac = mass / (r2 * r);      //Acceleration of particle outside 'Plummer sphere' 
    }
    else
	  {
      #ifdef UNEQUALSOFTENINGS
    ...
    tabindex = (int) (asmthfac * r);       // = r/(2*r_s) from eqn 21

		//Add result to total accelerations.
    if(tabindex < NTAB)
	  {
	    fac *= shortrange_table[tabindex];   //component from eqn.21, recall F=-grad(phi)
	    acc_x += dx * fac;
	    acc_y += dy * fac;
	    acc_z += dz * fac;
	    ninteractions++;
 ...
 */
int force_treeevaluate_ewald_correction(int target, int mode, double pos_x, 
                                        double pos_y, double pos_z, double aold)
{
  struct NODE *nop = 0;
  int no, cost;
...
void force_treeallocate(int maxnodes, int maxpart)
{
  ...
  if(first_flag == 0)
  {
    ...
    /* Initialize lookup table from phi (eqn 21). Recall that F = ma = -grad(phi).
       Below is grad(eqn 21). I believe that eqn 21 should not be a negative
    */
    for(i = 0; i < NTAB; i++)
    {
	    u = 3.0 / NTAB * (i + 0.5);
