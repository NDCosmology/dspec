 ...
 *	
 *	Writes the indices ('p') to the global array Ngblist[]. This is then used
 *	in hydra.c Returns the total number of nearby particles.
 *
 *	Actually this walks the ENTIRE particle tree. This includes ALL the
...
{
  int k;
	int no;									//Index to walk tree
	int p;									//Index to walk tree
	int numngb;							//Index to Ngblist[]
  FLOAT hdiff;						//Difference in smoothing length
  FLOAT searchmin[3];			//center - smoothing length
	FLOAT searchmax[3];			//center + smoothing length
  struct NODE *this;
	...
	//Construct upper and lower searching limits for Non-Periodic B.C.
  ...
  while(no >= 0)
  {
		//LOCAL particles are in P[0] -> P[NumPart-1] < P[All.MaxPart]
    if(no < All.MaxPart)	/* single particle */
  	{
  	  p = no;
  	  no = Nextnode[no];       

			//Ignore non-SPH particles
  	  if(P[p].Type > 0)
  	    continue;
  	  ...
  	  if(hdiff < 0)
  	    hdiff = 0;
			...
			//Check to make sure P[p] is within a smoothing length. If false, continue 
			#ifdef PERIODIC
			...
			//Add index to list of near neighbor particles.
  	  Ngblist[numngb++] = p;

			//If true end of Ngblist[] reached, do second loop.
  	  if(numngb == MAX_NGB)
	    {
	      printf
		...
		//For NON-LOCAL pseudo particles close by
    else
	  {
	    if(no >= All.MaxPart + MaxNodes)						/* pseudo particle */
	    {
	      ...
	      continue;
			...
			//Find difference in smoothing lengths between P[target] and Node[no]
	    this = &Nodes[no];
	    ...
	    no = this->u.d.sibling;	/* in case the node can be discarded */
			...
			//See if Nodes[no] is far from target, if true skip Nodes[no]
			#ifdef PERIODIC
 ...
 *  calling routine.
 *
 *	This writes the index of each 'close by' SphP to Ngblist[]
 */
