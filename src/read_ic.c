...
/*	#ifndef ISOTHERM_EQS
	...
	#endif
*/
	//Initialize global and local particle variables
  NumPart = 0;
  N_gas = 0;
  All.TotNumPart = 0;
	#ifdef SFR
		All.TotN_star = 0;				//GLOBAL number of stars
		N_star = 0;								//LOCAL  number of stars
	#endif
  ...
  /************* When there are more IC files than processors ****************/
  ...
  /*********** When (number of IC files) <= (Number of processors) *************/
  if(rest_files > 0)
  {
    ...
    for(gr = 0; gr < ngroups; gr++)
  	{
  	  if((filenr / All.NumFilesWrittenInParallel) == gr)	/* ok, it's this processor's turn */
  	    read_file(buf, masterTask, lastTask);   //Read in Data
  	  MPI_Barrier(MPI_COMM_WORLD);
  	}
  }
    ...
    if(All.InitGasTemp > 0)
	  {
	    ...
	    u_init *= All.UnitMass_in_g / All.UnitEnergy_in_cgs;	/ * unit conversion * /
	    	...
	    	u_init *= (1.0 / GAMMA_MINUS1);
				...
				/ * Photoionized even at low temps due to UV-Background. Self-shielding ignored
					 see p1214 of Schaye & Dalla Vecchia 2007* /
				#ifndef UVBACKGROUND
			...
			//This ONLY initializes All.InitGasTemp Entropy wasn't specified in the IC file
	    for(i = 0; i < N_gas; i++)
	    {
	      if(SphP[i].Entropy == 0)
  ...
  */
  ...
  if(ThisTask == 0)
  {
	...
	/* Must Change, so that gas masses in snapshot are correct. If All.MassTable[0] > 0
		 then, the gas masses are NOT written to the file, they are just infered from 
		 the header. However when feedback IS enabled, the gas mass CAN change. Hence Fix
		 below
	*/
	#ifdef FEEDBACK
		All.MassTable[0] = 0;	
...
void empty_read_buffer(enum iofields blocknr, int offset, int pc, int type)
{
  ...
  //Write data to P[]. depending on which case you are doing
  switch (blocknr)
  {
...
void read_file(char *fname, int readTask, int lastTask)
{
  ...
  {
    if(All.ICFormat == 1 || All.ICFormat == 2)        //Specified in *.param file
	  {
	    if(!(fd = fopen(fname, "r")))                   //Open IC data file
	    {
	    ...
	    SKIP;
	    my_fread(&header, sizeof(header), 1, fd);       //Read File header.
	    SKIP2;
    ...
    //Send header info to other processors
    for(task = readTask + 1; task <= lastTask; task++)
    	MPI_Ssend(&header, sizeof(header), MPI_BYTE, task, TAG_HEADER, MPI_COMM_WORLD);
  }
  else
    MPI_Recv(&header, sizeof(header), MPI_BYTE, readTask, 
             TAG_HEADER, MPI_COMM_WORLD, &status);
  ...
  if(All.TotNumPart == 0)
  {
    ...
    All.MaxPartSph = All.PartAllocFactor * (All.TotN_gas / NTask);	/* sets the maximum number of particles that may reside on a processor */

    //Let's set remaining All.* fields.
    ...
    if((RestartFlag == 2) || (RestartFlag == 0))
  ...
  if(ThisTask == readTask)
  {
    ...
    printf("\nreading file `%s' on task=%d (contains %d particles.)\n"
	     ...
	     "Type 5 (bndry): %8d  (tot=%6d%09d) masstab=%g\n\n", fname, ThisTask, n_in_file,
       readTask, lastTask, header.npart[0], (int) (header.npartTotal[0] / 1000000000),
  ...
  /*ALI: This memmove confuses me, I don't think any data has been read in.*/
	      ...
	      for(type = 0, offset = 0; type < 6; type++)
		    {
		      ...
		      else
		      {
                ...
                //Receive particle data from other processors
                ...
                //Empty particle data from buffer to P[].*
			          if(ThisTask == task)
				        {
 ...
 */
void distribute_file(int nfiles, int firstfile, int firsttask, 
                     int lasttask, int *filenr, int *master, int *last)
{
