...
void allocate_commbuffers(void)
{
  ...
  All.BunchSizeForce =
           (All.BufferSize * 1024 * 1024) / (sizeof(struct gravdata_index) + 
  ...
  All.BunchSizeDensity =
           (All.BufferSize * 1024 * 1024) / (2 * sizeof(struct densdata_in) + 
           2 * sizeof(struct densdata_out));
  ...
  All.BunchSizeHydro =
           (All.BufferSize * 1024 * 1024) / (2 * sizeof(struct hydrodata_in) + 
           2 * sizeof(struct hydrodata_out));
  ...
  HydroDataPartialResult = HydroDataResult + All.BunchSizeHydro;
	...
	/*
		GasPartSearchDataIn, GasPartSearchDataGet, GasPartSearchDataResult and 
		GasPartSearchDataPartialResult all point to different sections of the 
		CommBuffer. NOTE : All.BufferSize is the size of CommBuffer in MB.
	*/
	All.BunchSizeGasSearch =
					(All.BufferSize * 1024 * 1024) / (2 * sizeof(struct gaspartsearchdata_in) + 
					2 * sizeof(struct gaspartsearchdata_out));

	GasPartSearchDataIn 			= (struct gaspartsearchdata_in *) CommBuffer;
	GasPartSearchDataGet 		 	= GasPartSearchDataIn + All.BunchSizeGasSearch;
	GasPartSearchDataResult 	= (struct gaspartsearchdata_out *)(GasPartSearchDataGet + All.BunchSizeGasSearch);
	GasPartSearchDataPartialResult = GasPartSearchDataResult + All.BunchSizeGasSearch;


	/*
		FeedbackDataIn, FeedbackDataGet, FeedbackDataResult and 
		FeedbackDataPartialResult all point to different sections of the 
		CommBuffer. NOTE : All.BufferSize is the size of CommBuffer in MB.
	*/
	All.BunchSizeFeedback =
					(All.BufferSize * 1024 * 1024) / (2 * sizeof(struct feedbackdata_in) + 
					2 * sizeof(struct feedbackdata_out));

	FeedbackDataIn 			= (struct feedbackdata_in *) CommBuffer;
	FeedbackDataGet 		 	= FeedbackDataIn + All.BunchSizeFeedback;
	FeedbackDataResult 	= (struct feedbackdata_out *)(FeedbackDataGet + All.BunchSizeFeedback);
	FeedbackDataPartialResult = FeedbackDataResult + All.BunchSizeFeedback;


		/*
			FeedbackNormDataIn, FeedbackNormDataGet, FeedbackNormDataResult and 
			FeedbackNormDataPartialResult all point to different sections of the 
			CommBuffer. NOTE : All.BufferSize is the size of CommBuffer in MB.
		*/
		All.BunchSizeNormalization = 
					(All.BufferSize * 1024 * 1024 / (2 * sizeof(struct feedbacknormdata_in) +
					2 * sizeof(struct feedbacknormdata_out)));
	
		FeedbackNormDataIn  = (struct feedbacknormdata_in *) CommBuffer;
		FeedbackNormDataGet = FeedbackNormDataIn + All.BunchSizeNormalization;
		FeedbackNormDataResult = (struct feedbacknormdata_out *)(FeedbackNormDataGet + 
															All.BunchSizeNormalization);
		FeedbackNormDataPartialResult = FeedbackNormDataResult + All.BunchSizeNormalization;



  All.BunchSizeDomain =
           (All.BufferSize * 1024 * 1024) / (sizeof(struct particle_data) + 
    ...
    if(!(P = malloc(bytes = All.MaxPart * sizeof(struct particle_data))))
  	{
		...
		#ifdef SFR
    	StarP = malloc(sizeof(struct particle_data));
		#endif
